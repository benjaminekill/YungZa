...
//			
			...
			for(Faculty facttmp:facultyList){
				...
				for(Department deptmp:departmentList){					
					...
					//if( !"12".equals(deptmp.getDepartmentId())){  // 05: com, 12:เธชเธฒเธฃเธฐเธชเธ�เน€เธ—เธจ
					...
					//	logger.info(teacherListWSTmp.getTeacherId()+":"+teacherListWSTmp.getTeacherTname()+" :"+teacherListWSTmp.getTeacherEname());
						if(teacherListWSTmp.getTeacherId()==null){
							...
							AcademicKPIUserMapping headApproveSummary = new AcademicKPIUserMapping();
							headApproveSummary.setAcademicYear(academicYear);
							headApproveSummary.setFac_code(facultyCode);
							headApproveSummary.setFac_name(facttmp.getFacultyTname());
							headApproveSummary.setDep_code(deptmp.getDepartmentId());
							headApproveSummary.setDep_name(deptmp.getDepartmentName());
							
							headApproveSummary.setThaiName(teacherListWSTmp.getTeacherTname());
							
							
							pbpTeacher.setHeadApproveSummary(headApproveSummary);
	...
	private void getAndInsertTeachTableWS(int semester,int degree ,List<Teacher> teacherListIn){
		...
		//	if(!"31110".equals(teacherTmp.getTeacherIdStr())){ // เธญเธณเธ�เธฒเธ�
				
				
			//}else{
				
				AcademicKPIUserMapping appSummary2 = teacherTmp.getHeadApproveSummary();
				...
				facultyDao.updateTeachTableWS(semester,teachTableResponseList,degree,facultyCode,academicYear, appSummary2);
